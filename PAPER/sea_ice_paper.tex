...
0.391/0.511/0.485, surpassing LISA-7B, CLIPSeg, and GeoPixel-7B. We report all
results transparently, including the Old Ice failure (F1\,=\,0.000) traced to
single-band modality limits.
\end{abstract}
...
as surveyed by Liu et al.~\cite{liu2021deep}. ResNet-based classifiers
\cite{he2016deep} were applied to multi-temporal Sentinel-1 scenes, while
several groups adapted U-Net \cite{ronneberger2015unet} to produce pixel-level
sea ice concentration maps \cite{wang2021sarunet}. A notable limitation shared by all of these efforts is
...
\begin{table*}[t]
\centering
\caption{Comparison against segmentation baselines on the 90-image SAR sea ice
         test split. All methods share the same evaluation harness and
         Otsu-binarised ground truth.
         \dag~Zero-shot; no in-domain SAR training.
         \ddag~Result invalid due to a mask-path bug in training
               (see Section~\protect\ref{sec:comparison_analysis}).
         Bold marks the column best among valid rows.}
\label{tab:comparison}
\small
\resizebox{\linewidth}{!}{%
\begin{tabular}{llccccc}
\toprule
\textbf{Method} & \textbf{Type} & \textbf{Lang.} & \textbf{SAR-trained} & \textbf{gIoU} & \textbf{cIoU} & \textbf{Dice} \\
\midrule
\textbf{Ours} & Joint seg.\ + cls.\ (proposed) & \checkmark & \checkmark & \textbf{0.391} & \textbf{0.511} & \textbf{0.485} \\
LISA-7B\dag\ \cite{lai2024lisa}           & Reasoning seg.\ LMM      & \checkmark & $\times$ & 0.255 & 0.290 & 0.349 \\
CLIPSeg\dag\ \cite{luddecke2022image}     & Zero-shot text$\to$mask  & \checkmark & $\times$ & 0.084 & 0.123 & 0.127 \\
GeoPixel-7B\dag\ \cite{geopixel2024}     & Remote-sensing LMM       & \checkmark & $\times$ & 0.032 & 0.035 & 0.060 \\
DeepLabv3+\ddag\ \cite{chen2018deeplabv3plus} & Supervised CNN seg.  & $\times$   & \checkmark & 0.001 & 0.001 & 0.002 \\
\bottomrule
\end{tabular}}
\end{table*}

Our model leads on all three metrics among valid rows. The gap over LISA-7B is
particularly notable: 0.391 vs 0.255 gIoU — a 53\% relative improvement —
from a model with roughly 4,500 times fewer trainable parameters. The comparison
is deliberately unfair in our direction (we have in-domain training; they do
not), and Section~\ref{sec:comparison_analysis} discusses this nuance.
GeoPixel's low scores despite remote-sensing specialisation (0.032 gIoU)
illustrate that optical-RS pretraining does not transfer to SAR scattering maps.
The DeepLabv3+ row is included for completeness but is \textbf{not a valid
result}: its training dataset loaded incorrect mask paths, producing all-zero
training masks; DeepLab therefore learned to predict empty masks. The entry
is shown as-is so the bug is on record; a corrected run will be reported in
the final version.
