...
\title{Language-Guided Simultaneous Segmentation and Six-Class\\
       Type Classification of Sea Ice from Synthetic Aperture\\
       Radar Imagery: A Proof-of-Concept Study}

\author[dtu]{Prakhar Mishra\corref{cor1}}
\cortext[cor1]{Corresponding author.}
\address[dtu]{Department of Information Technology, Delhi Technological University,
              Shahbad Daulatpur, Main Bawana Road, New Delhi 110042, India}
\ead{prakharmishra_24phdit502@dtu.ac.in}

%%-----------------------------------------------------------------
\begin{abstract}
Autonomous monitoring of sea ice extent and type is foundational to climate
science, polar navigation, and ecological research, yet the remotely sensed
data best suited to this task — single-channel grayscale Synthetic Aperture
Radar (SAR) imagery — presents formidable analytical challenges. Speckle
noise, radiometric ambiguity across ice types, and the absence of colour or
texture cues that underpin optical-image classifiers each limit how far
conventional deep-learning pipelines can take us. Crucially, no prior work
has tackled the combined problem of pixel-level segmentation \emph{and}
multi-class ice-type recognition within a single trainable framework on SAR
data.

This paper introduces the first proof-of-concept pipeline for this joint task.
Our architecture fuses a frozen CLIP ViT-L/14 vision backbone — augmented with
rank-8 Low-Rank Adaptation (LoRA) adapters touching only 1.57 million of 304
million parameters (0.52\%) — with natural-language scene descriptions through
four stacked cross-attention blocks. A lightweight image-conditioned U-Net
decoder produces binary ice/water segmentation masks, while a two-layer MLP
head performs six-class ice-type classification, both trained jointly with a
combined Focal-Tversky segmentation loss and cross-entropy classification loss.

Trained on 420 SAR scenes drawn from six ice categories and evaluated on a
deterministic 90-image held-out test split, the model attains mIoU\,=\,0.351,
pixel accuracy\,=\,0.656, and weighted-F1\,=\,0.778 for classification. Under
a shared comparison harness (gIoU / cIoU / Dice on the same 90 images), our
approach scores 0.391\,/\,0.511\,/\,0.485, outperforming LISA-7B
(0.255\,/\,0.290\,/\,0.349), CLIPSeg (0.084\,/\,0.123\,/\,0.127), and
GeoPixel-7B (0.032\,/\,0.035\,/\,0.060). We report all results transparently,
including the complete failure on Old Ice classification (F1\,=\,0.000) traced
to single-band modality limitations, and discuss quantitative paths forward
via dual-polarisation inputs. To the best of our knowledge, this is the first
publicly reproducible baseline combining language guidance with multi-task
SAR sea ice understanding.
\end{abstract}
...
\citet{liu2021deep} trained a ResNet-based classifier on multi-temporal
...
\begin{figure}[H]
  ...
  \fbox{\parbox{0.9\linewidth}{\centering
    \vspace{3cm}
    \textbf{[FIGURE PLACEHOLDER — fig:dataset\_samples]}\\[4pt]
    Six representative SAR patches (one per ice class).
    Arrange as a $2 \times 3$ grid. Each panel: raw SAR image (top),
    Otsu-binarised scattering mask (bottom), class name as subtitle.
    Greyscale colourmaps; no artificial colour added.
    \vspace{3cm}
  }}
...
\begin{figure}[H]
  ...
  \fbox{\parbox{0.9\linewidth}{\centering
    \vspace{4cm}
    \textbf{[FIGURE PLACEHOLDER — fig:architecture]}\\[4pt]
    Full pipeline architecture diagram — see \texttt{figure\_prompts.txt}
    for the detailed Gemini AI generation prompt.
    \vspace{4cm}
  }}
...
\begin{figure}[H]
  ...
  \fbox{\parbox{0.9\linewidth}{\centering
    \vspace{2.5cm}
    \textbf{[FIGURE PLACEHOLDER — fig:preprocessing]}\\[4pt]
    Three-panel horizontal strip: (a) raw SAR JPEG, (b) Lee-filtered
    result, (c) dB-scaled pseudo-RGB tensor.
    Show intensity histograms below each panel.
    \vspace{2.5cm}
  }}
...
\caption{Comparison against language-guided segmentation baselines on the
         90-image SAR sea ice test split (gIoU / cIoU / Dice).
         All methods use the same evaluation harness and Otsu-binarised ground truth.
         \dag~Operates zero-shot without any in-domain SAR training.
         Bold marks the column best.}
\label{tab:comparison}
\begin{tabular}{lcccccc}
\toprule
\textbf{Method} & \textbf{Family} & \textbf{SAR-trained} & \textbf{Parameters} & \textbf{gIoU} & \textbf{cIoU} & \textbf{Dice} \\
\midrule
\textbf{Ours} & Lang.-guided seg. + cls. & \checkmark & 1.57M trainable & \textbf{0.391} & \textbf{0.511} & \textbf{0.485} \\
LISA-7B\dag\ \cite{lai2024lisa}       & Reasoning seg. LMM   & $\times$ & 7B & 0.255 & 0.290 & 0.349 \\
CLIPSeg\dag\ \cite{luddecke2022image} & Zero-shot text→mask  & $\times$ & 168M & 0.084 & 0.123 & 0.127 \\
GeoPixel-7B\dag\ \cite{geopixel2024} & Remote-sensing LMM   & $\times$ & 7B & 0.032 & 0.035 & 0.060 \\
\bottomrule
\end{tabular}
\end{table*}
...
\caption{Ablation study on the 90-image test split.
         \textsuperscript{*}Ablated variants used $\leq$15 training epochs (patience 5);
         the full model ran for 50 epochs. Bold marks the column best.
         Daggers (\dag) mark results where the training-budget difference
         may be a material confounder.}
\label{tab:ablation}
\begin{tabular}{lccc p{5.5cm}}
\toprule
\textbf{Variant} & \textbf{mIoU} & \textbf{Dice} & \textbf{Weighted F1} & \textbf{Note} \\
\midrule
Full Model (Focal + Tversky + LoRA + Aux)
  & 0.351 & 0.442 & 0.778 & Published configuration \\
\addlinespace
w/o LoRA
  & 0.365 & 0.469 & 0.534 & LoRA boosts F1 by 0.244; slightly hurts seg. \\
Token Decoder\dag
  & 0.344 & 0.455 & 1.000 & Comparable seg.; F1=1.0 on 15-sample set \\
w/o Aux Loss
  & 0.324 & 0.405 & 0.801 & Aux supervision improves mIoU by 0.027 \\
w/o Depth Encoder
  & 0.351 & 0.442 & 0.778 & Bit-identical — depth adds nothing \\
\addlinespace
Focal Only
  & 0.252 & 0.310 & 1.000 & Collapses segmentation \\
\textbf{Tversky Only}
  & \textbf{0.389} & \textbf{0.493} & 0.989 & \textbf{Best segmentation row} \\
BCE + Dice
  & 0.332 & 0.421 & 0.822 & Standard baseline \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{figure}[H]
  ...
  \fbox{\parbox{0.9\linewidth}{\centering
    \vspace{3cm}
    \textbf{[FIGURE PLACEHOLDER — fig:per\_class\_f1]}\\[4pt]
    Horizontal bar chart. Six bars, one per class. X-axis: F1 score 0 to 1.
    Colour-code: green for F1=1.0, orange for 0.5–1.0, red for F1=0.
    Order (top to bottom): Glaciers (1.0), Icebergs (1.0), Young Ice (1.0),
    Floating Ice (1.0), First Year Ice (0.667), Old Ice (0.0).
    Annotate each bar with its numeric F1 value.
    \vspace{3cm}
  }}
...
\begin{figure}[H]
  ...
  \fbox{\parbox{0.9\linewidth}{\centering
    \vspace{4cm}
    \textbf{[FIGURE PLACEHOLDER — fig:qualitative]}\\[4pt]
    $6 \times 3$ grid. Rows = one sample per ice class (order as above).
    Columns: (1) Raw SAR input, (2) Ground truth binary mask,
    (3) Predicted binary mask with overlay (green = TP, red = FP, blue = FN).
    Add per-row class label and IoU value.
    \vspace{4cm}
  }}
  \caption{Qualitative segmentation results — one sample per ice class.
           ...
           (\textcolor{green}{green} = TP, \textcolor{red}{red} = FP,
           \textcolor{blue}{blue} = FN).
           The model captures broad ice extent but misses fine boundaries
           (Boundary IoU $\approx$ 0.014), consistent with the coarse
           Otsu supervision signal.}
...
\begin{figure}[H]
  ...
  \fbox{\parbox{0.9\linewidth}{\centering
    \vspace{3cm}
    \textbf{[FIGURE PLACEHOLDER — fig:training\_curves]}\\[4pt]
    Three vertically stacked subplots sharing x-axis (epoch 0–50):
    (top) total loss: train and val; (middle) mIoU: train and val;
    (bottom) weighted F1: train and val.
    Highlight epoch of best val mIoU with a vertical dashed line.
    Legend inside each subplot.
    \vspace{3cm}
  }}
  \caption{Training dynamics over 50 epochs. Train and validation mIoU
           converge to similar values throughout training, indicating that
           the performance ceiling is a property of the supervision signal
           (Otsu-binarised scattering maps) rather than of overfitting.}
...
\begin{figure}[H]
  ...
  \fbox{\parbox{0.9\linewidth}{\centering
    \vspace{3cm}
    \textbf{[FIGURE PLACEHOLDER — fig:confusion]}\\[4pt]
    $6\times6$ normalised confusion matrix heatmap.
    Classes (row = true, col = predicted):
    Young Ice, First Year Ice, Floating Ice, Glaciers, Icebergs, Old Ice.
    Colour: dark blue = high, white = low. Annotate each cell with fraction.
    \vspace{3cm}
  }}
...
\begin{figure}[H]
  ...
  \fbox{\parbox{0.9\linewidth}{\centering
    \vspace{2.5cm}
    \textbf{[FIGURE PLACEHOLDER — fig:threshold]}\\[4pt]
    Dual-axis line chart. X-axis: threshold 0.2 to 0.8.
    Left Y-axis (solid lines): mIoU and Dice (0 to 0.6).
    Right Y-axis (dashed lines): Precision and Recall (0 to 1.0).
    Mark the operating threshold (0.5) with a vertical dashed line.
    \vspace{2.5cm}
  }}
...
A striking property of the training dynamics (Figure~\ref{fig:training_curves})
is that train mIoU and validation mIoU remain nearly identical throughout all
50 epochs. This absence of a train--val gap means the model is not overfitting;
